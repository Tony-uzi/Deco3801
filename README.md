...
### Shapez Mod Installation
Place the latest `mymodv6.js` file into the `mods/` folder. You can locate the `mods/` folder by clicking the "Open Mods Folder" button in the Shapez Standalone version. Ensure that you have selected the 1.5.0-modloader branch in Steam.

### Additional Dependencies
Please make sure to install the following dependencies for both Python and JavaScript as required.
...
### Data Transmission
Use `websocket_server.py` to perform data communication between the JS and Python side (only for the initial version).
