...
function translator(line) {
    ...
    if (lineParts.length < 2) {
        console.log(`错误：在行 '${line}' 中找不到操作符。`);
        return;
    }
    ...
    switch (operation) {
        case "STACK":
            readableLine += "\nStack all the shapes we have build and form the shape ahead";
            break;
        case "RAW":
            readableLine += "\nPlace the shape ahead";
            break;
        case "ROTATE_1":
            readableLine += "\nRotate 1 quadrant clockwise and form the shape ahead";
            break;
        case "ROTATE_2":
            readableLine += "\nRotate 2 quadrant clockwise and form the shape ahead";
            break;
        case "ROTATE_3":
            readableLine += "\nRotate 3 quadrant clockwise and form the shape ahead";
            break;
    }

    console.log(readableLine);  // 打印出翻译后的结果
}
